  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
        <div className="text-center"><div className="text-4xl mb-2">🗑️</div><h3 className="font-bold text-slate-800 text-lg">طلب حذف</h3><p className="text-sm text-slate-500">{employee.fullName}</p><p className="text-xs text-slate-400 font-mono">{employee.nationalNumber}</p></div>
        <div className="mt-4 flex gap-2"><button onClick={onClose} className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm">إلغاء</button><button onClick={async () => { const ok = await requestEmployeeDelete({ nationalNumber: employee.nationalNumber, employeeName: employee.fullName, reason: "طلب حذف", docNumber: "", docDate: "", submittedBy: session.fullName }); if (ok) { addLog(session, "delete_user", `طلب حذف: ${employee.fullName}`); window.dispatchEvent(new Event("delete-requests-changed")); onSuccess(); } }} className="flex-1 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium">تأكيد الحذف</button></div>
      </div>
    </div>
  );
}
