...
export async function updateDeleteRequest(refNum: string, updates: Partial<DeleteRequest>): Promise<{ status: string; message?: string }> {
  try {
    await fetch(API_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain" }, body: JSON.stringify({ action: "update_delete_request", refNum, updates }) });
    return { status: "success" };
  } catch { return { status: "error", message: "فشل الاتصال" }; }
}

export async function deleteDeleteRequest(refNum: string): Promise<{ status: string; message?: string }> {
  try {
    await fetch(API_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain" }, body: JSON.stringify({ action: "delete_delete_request", refNum }) });
    return { status: "success" };
  } catch { return { status: "error", message: "فشل الاتصال" }; }
}

export async function permanentDeleteArchive(nationalNumber: string, deletedBy: string): Promise<{ status: string; message?: string }> {
  try {
    await fetch(API_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain" }, body: JSON.stringify({ action: "permanent_delete_archive", nationalNumber, deletedBy }) });
    return { status: "success", message: "تم الحذف النهائي بنجاح" };
  } catch { return { status: "error", message: "فشل الاتصال" }; }
}

export async function getSystemInfo(): Promise<{ status: string; info?: Record<string, string>; message?: string }> {
  try {
    const res = await fetch(`${API_URL}?action=get_system_info&t=${Date.now()}`);
    const data = await res.json();
    return data.status === "success" ? data : { status: "error", message: "فشل جلب المعلومات" };
  } catch { return { status: "error", message: "فشل الاتصال" }; }
}
...
export function findEmployeeByNationalNumber(
  ...
  return employees.find((e) => e.nationalNumber === nn) || null;
}
