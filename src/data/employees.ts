...
async function sendAction(payload: ActionPayload): Promise<boolean> {
  try {
    await fetch(API_URL, {
      method: "POST",
      mode: "no-cors",
...
export function getMissingFields(emp: Employee): string[] {
  ...
  Object.entries(REQUIRED_FIELD_LABELS).forEach(([key, label]) => {
    const value = emp[key as keyof Employee];
    if (!value || (typeof value === "string" && value.trim() === "" || value.trim() === "-" || value.trim() === "تحت الاجراء")) {
      missing.push(label);
    }
  });
  return missing;
...
export function findEmployeeByNationalNumber(employees: Employee[], nn: string): (Employee & Record<string, string>) | null {
  ...
  return { ...found } as Employee & Record<string, string>;
}

export function syncCustomFieldsFromSheet(employees: any[]): void {
  const existing = JSON.parse(localStorage.getItem("nacc_custom_fields_v1") || "[]");
  const existingLabels = existing.map((f: any) => f.label);
  employees.forEach((emp) => {
    Object.keys(emp).forEach((key) => {
      if (key.startsWith("field_custom_") && !existingLabels.includes(key)) {
        const newField = { id: `f_${Date.now()}`, key, label: key, isRequired: false, createdBy: "system", createdAt: new Date().toISOString() };
        existing.push(newField);
        existingLabels.push(key);
      }
    });
  });
  localStorage.setItem("nacc_custom_fields_v1", JSON.stringify(existing));
}
