...
export function LogsTab() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      r = r.filter((l) => l.fullName.toLowerCase().includes(s) || l.username.toLowerCase().includes(s) || l.details.toLowerCase().includes(s));
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={() => { if (confirm("حذف كل السجلات؟")) { clearLogs(); window.location.reload(); } }}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 flex flex-wrap gap-2">
        <input type="text" placeholder="بحث بالاسم أو التفاصيل..." value={search}
        ...
        <span className="text-xs text-slate-500 self-center">{filtered.length} سجل</span>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {paginated.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-slate-400">لا توجد سجلات</td></tr>
              )}
            </tbody>
          </table>
        </div>
